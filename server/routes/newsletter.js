...
// Reset exported_at so subscriber is included in next "Export New" (admin only)
router.post("/subscribers/:id/mark-new", requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;

    const result = await pool.query(
      `UPDATE newsletter_subscribers SET exported_at = NULL WHERE id = $1
    ...
    logger.info("Subscriber marked as new", { subscriberId: id, userId: req.user.id });
    res.json(result.rows[0]);
  } catch (error) {
    logger.error("Error marking subscriber as new", { subscriberId: req.params.id, userId: req.user.id, error: error.message, stack: error.stack });
    res.status(500).json({ message: "Failed to mark subscriber as new" });
  }
});
